        ...
        switch (last(state.soyState)) {
          ...
          case "templ-ref":
            if (match = stream.match(/(\.?[a-zA-Z_][a-zA-Z_0-9]+)+/)) {
              state.soyState.pop();
              // If the first character is '.', it can only be a local adminlte.
              if (match[0][0] == '.') {
                return "variable-2"
              }
              // Otherwise
          ...
          case "tag":
            ...
            if (stream.match(/^\/?}/)) {
              ...
              if (state.tag == "/adminlte" || state.tag == "/deltemplate") {
