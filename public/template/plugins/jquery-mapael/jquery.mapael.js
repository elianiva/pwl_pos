...
(function (factory) {
    ...
    Mapael.prototype = {
        ...
        drawLink: function (id, xa, ya, xb, yb, elemOptions) {
            ...
            // template(c,d) = template(a,b) (=abDist)
            var abDist = Math.sqrt((xb - xa) * (xb - xa) + (yb - ya) * (yb - ya));

            // Solution for equation template(cd) = sqrt((xd - xc)² + (yd - yc)²)
            // template(c,d)² = (xd - xc)² + (yd - yc)²
            // We assume that template(c,d) = template(a,b)
            // so : (xd - xc)² + (yd - yc)² - template(a,b)² = 0
            // With the factor : (xd - xc)² + (yd - yc)² - (factor*template(a,b))² = 0
            // (xd - xc)² + (acd*xd + bcd - yc)² - (factor*template(a,b))² = 0
            var a = 1 + acd * acd;
